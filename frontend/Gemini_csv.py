...
def documentazione_page():
    ...
    uploaded_files = st.file_uploader("Carica documenti", type=["pdf"], accept_multiple_files=True)
    if uploaded_files:
        ...
        for uploaded_file in uploaded_files:
            ...
            # Prepara i dati per la richiesta (il percorso del file)
            payload = {"file_path": temp_file_path}

            # Effettua la richiesta POST a FastAPI
            try:
              response = requests.post("http://localhost:8000/process_pdf/", json=payload) #modifica
              response.raise_for_status()  # Solleva un'eccezione per codici di stato HTTP non-2xx
              result = response.json()
              st.success(result["message"])  # Mostra il messaggio di successo
              # if result["result"]:  # Se il tuo agent restituisce un risultato, mostralo
              #   st.write("Risultato elaborazione:", result["result"])
            except requests.exceptions.RequestException as e:
              st.error(f"Errore durante l'elaborazione: {e}")

            # Opzionale: rimuovere il file temporaneo dopo la richiesta
            # import os
